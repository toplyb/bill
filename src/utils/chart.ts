...
const getAllMonthOrAllYear = (type: ButtonType) => {
  ...
  billList.forEach(item => {
    ...
    if (type === 'year') {
      ...
      if (tempData[year]) {
        tempData[year][item.type] += Number(item.money)
      } else {
        tempData[year] = {
          expense: item.type === 'expense' ? Number(item.money) : 0,
          income: item.type === 'income' ? Number(item.money) : 0
        }
      }
    } else if (type === 'month') {
      const month = dateString[1]
      if (tempData[month]) {
        tempData[month][item.type] += Number(item.money)
      } else {
        tempData[month] = {
          expense: item.type === 'expense' ? Number(item.money) : 0,
          income: item.type === 'income' ? Number(item.money) : 0
